...
const Store = types
  ...
  .actions(self => {
    ...
    const fetchStarwars = flow(function*() {
      const response = yield axios.get("https://swapi.dev/api/films/1/");
      console.log("fetchStarwars", response, window);
    ...
    return { start, stop, update, fetchStarwars };
  });
